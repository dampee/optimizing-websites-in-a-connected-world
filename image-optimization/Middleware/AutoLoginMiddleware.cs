...
namespace image_optimization.Middleware;

/// <summary>
/// Auto login if you are on a local computer or network. Not for production use.
/// Original from https://github.com/Infocaster/UrlTracker/blob/v10/develop/test/UrlTracker.Resources.Website/Middleware/AutologinMiddleware.cs
/// </summary>
public class AutoLoginMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext httpContext, IBackOfficeSignInManager signInManager,
        IBackOfficeUserManager backOfficeUserManager, IUmbracoContextAccessor umbracoContextAccessor)
    {
        ...
        if (!IsAuthenticated(httpContext) && IsBackofficeRequest(umbracoContextAccessor) && RequestIsLocal(httpContext))
        {
            ...
            if (user == null)
            {
                throw new InvalidOperationException("Could not find Super User (with id " + Constants.Security.SuperUserIdAsString + ")");
            }
    ...
    {
        return umbracoContextAccessor.TryGetUmbracoContext(out var umbracoContext) &&
    ...
    private static bool RequestIsLocal(HttpContext httpContext)
    {
        ...
        //    (read as: connect from same device or device on the local network)
